...
label_font = pygame.font.Font('Roboto-Condensed.ttf', 32)
medium_font = pygame.font.Font('Roboto-Condensed.ttf', 24)
...
def draw_save_menu(beat_name, typing):
    ...
    screen.blit(exit_text, (WIDTH - 145, HEIGHT - 70))
...
def draw_load_menu(index):
    ...
    menu_text = label_font.render('SAVE MENU: Select a Beat To Load', True, white)
    screen.blit(menu_text, (400, 40))

    loading_btn = pygame.draw.rect(screen, gray, [WIDTH // 2 - 200, HEIGHT - 105, 400, 100], 0, 5)
    loading_txt = label_font.render('Load Beat', True, white)
    screen.blit(loading_txt, (WIDTH // 2 - 60, HEIGHT - 75))

    delete_btn = pygame.draw.rect(screen, gray, [WIDTH // 2 - 450, HEIGHT - 105, 200, 100], 0 , 5)
    delete_text = label_font.render('Delete Beat', True, white)
    screen.blit(delete_text, (WIDTH // 2 - 420, HEIGHT - 75))
    ...
    if 0 <= index < len(saved_beats):
        pygame.draw.rect(screen, light_gray, [190, 95 + index * 50 , 1000, 50])
...
while run:
    ...
    # bpm
    bpm_rect = pygame.draw.rect(screen, gray, [290, HEIGHT - 150, 200, 100], 5, 5)
    ...
    bpm_add_rect = pygame.draw.rect(screen, gray, [495, HEIGHT - 150, 48, 48], 0, 5)
    bpm_sub_rect = pygame.draw.rect(screen, gray, [495, HEIGHT - 100, 48, 48], 0, 5)

    add_text = medium_font.render('+5', True, white)
    sub_text = medium_font.render('-5', True, white)
    screen.blit(add_text, (505, HEIGHT - 140))
    screen.blit(sub_text, (505, HEIGHT - 90))

    # beats
    beats_rect = pygame.draw.rect(screen, gray, [575, HEIGHT - 150, 200, 100], 5, 5)
    ...
    beats_add_rect = pygame.draw.rect(screen, gray, [780, HEIGHT - 150, 48, 48], 0, 5)
    beats_sub_rect = pygame.draw.rect(screen, gray, [780, HEIGHT - 100, 48, 48], 0, 5)

    add_text2 = medium_font.render('+1', True, white)
    sub_text2 = medium_font.render('-1', True, white)
    screen.blit(add_text2, (790, HEIGHT - 140))
    screen.blit(sub_text2, (790, HEIGHT - 90))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and typing:
            if event.key == 8 and len(beat_name) > 0:
